...
@section('breadcrumb', 'Dashboard  /  Produk /  Produk Terlaris')
...
<script>
    ...
    $(document).ready(async function () {
        ...
        table = $('#data-table').DataTable({
            ...
            order: [[2, 'DESC']],
            ajax:{
                url: "{{ env('API_URL') . '/product/data-table-top-product' }}",
                dataType: 'JSON',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
            ...
            columns: [
                {
                    ...
                    render: function(_,__,___){
                        ...
                        <label class="pure-material-checkbox">
                            <input type="checkbox" ${___.is_top_product ? 'checked' : ''} class="checked"/>
                        </label>
        ...
        $('#data-table tbody').on('click', '.checked', function () {
            ...
            if (this.checked) {
                ...
                const product_top  = await(await fetch(`{{ env('API_URL') . '/top-product/update' }}`, __propsPOST)).json();
