...
@section('title', '| Stok Produk Point')
@section('breadcrumb', 'Dashboard  /  Produk Point /  Stok Produk Point')

@section('content')


<button type="button" class="btn btn-primary btn-icon-text px-3 px-lg-4 btn-gradient float3" style="right:300px" onclick="openModalBulk()">
    &nbsp;<i class="link-icon" data-feather="plus-square"></i>
    Update Stock Bulk
</button>

<button class="btn btn-success btn-icon-text px-3 px-lg-4 float4" style="right: 40px" id="download-bulk">
    &nbsp;<i class="link-icon" data-feather="plus-square"></i>
    Export Stock Produk Point
</button>
		...
		<div class="card">
			<div class="card-header" @if(Session::get('role') != 'Super' && Session::get('role') != 'Admin Pusat') style="padding:1.35rem 1.5rem 0rem 1.5rem;" @else style="padding:1rem 1.5rem .3rem 1.5rem;" @endif>
				<div class="d-flex justify-content-between align-items-baseline">
					<h5 class="card-title"> Stok Produk Point </h5>
						...
						<div @if(Session::get('role') != 'Super' && Session::get('role') != 'Admin Pusat') style="display:none;" @endif>
							<select class="form-control select" id="warehouse">
								@foreach($warehouse as $a)
									...
									<option value="{{ $a->id }}" @if($a->id != '0') disabled @endif> {{ $a->code . ' - ' . $a->name . ' (' . $a->short . ')' }} </option>
								@endforeach
							</select>
							&nbsp;&nbsp;&nbsp;&nbsp;
						</div>
		...
		<div class="modal-content" style="border:none;">
			<form class="cmxform" id="stock-form" method="get" action="#" enctype="multipart/form-data">
				<div class="modal-header" style="background:rgb(42,143,204);background:linear-gradient(41deg, rgba(42,143,204,1) 0%, rgba(142,205,243,1) 50%);">
					<h5 class="modal-title" style="color:#fff;">Tambah Stok Produk Point</h5>
...
<script>
    ...
    $('#form-bulk').on('submit', function(e){
        ...
        $.ajax({
            type:'post',
            url:"{{ env('API_URL') }}/stock-product-point/import",
            contentType: false,
            processData: false,
            data:data,
    ...
    async function exportData(e) {
        ...
        $.ajax({
            url:`{{ env('API_URL') . '/stock-product-point/export' }}`,
            method:"POST",
            data: data,
            dataType:'JSON',
            contentType: false,
            ...
            success:function(result){
                ...
                if(status) {
                    ...
                    toastr.success(message, { fadeAway: 10000 });
                    elm_download_bulk.innerHTML = __iconPlus() + ' Export Stock Product Point';
	...
	function fDataTable(){
		...
		table = $('#dataTable').DataTable({
			...
			order: [[2, 'ASC']],
			ajax:{
				url: "{{ env('API_URL') . '/stock-product-point/data-table' }}",
				dataType: 'JSON',
				type: 'POST',
				data: {
					_token: '{{ csrf_token() }}',
	...
	$('#stock-form').on('submit', function(e){
		...
		_sendRequest('stock-product-point/update', 'POST', null, false, 'dataTable', null, 'stock-form');
